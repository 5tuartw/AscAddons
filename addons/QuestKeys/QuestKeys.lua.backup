...
end-- Handler: Spacebar for accept/continue/complete
local function HandleSpacebar()
    if not DB.spacebarAccept or isProcessingKey then return end
...
local function HandleNumberKey(num)
    if not DB.numberKeysSelect or isProcessingKey then return end
...
-- Handler: ESC to decline or close
local function HandleEscape()
    ...
    return false -- Let ESC do normal things
end
...
local function SetupKeyBindings()
    ...
    -- Update hints when quest content changes
    ...
    -- Hook ESC key for declining quests
...
SlashCmdList["QUESTKEYS"] = function(msg)
    ...
    if msg == "" or msg == "help" then
        ...
        print("  |cffFFFF00ESC|r - Decline quest")
