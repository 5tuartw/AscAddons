...
local VERSION = "1.6 - Fixed Numbering"
...
local function AddKeyHintToButton(button, keyNum)
    ...
    if originalText and not originalButtonTexts[button] then
        ...
        button:SetText("|cff0070dd[" .. keyNum .. "]|r " .. originalText)
    end
end
        ...
        if numOptions == 1 and numActive == 0 and numAvailable == 0 then
            ...
            if button then
                ...
                if text and (text:lower():find("goodbye") or text:lower():find("farewell")) then
                    ...
                    button:SetText("|cff0070dd[1]|r " .. originalButtonTexts[button] .. " |cffFFFFFF[SPACE]|r")
                end
            end
        end
...
local function HandleNumberKey(num)
    ...
    elseif GossipFrame:IsVisible() then
        ...
        elseif num <= numOptions + numActive then
            local questIndex = num - numOptions
            Debug("Selecting active quest", questIndex)
            SelectGossipActiveQuest(questIndex)
        elseif num <= numOptions + numActive + numAvailable then
            local questIndex = num - numOptions - numActive
            Debug("Selecting available quest", questIndex)
